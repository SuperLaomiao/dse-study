...
export default function BottomNav({ currentPath }: { currentPath: string }) {
  return (
    <nav aria-label="Primary" className="mt-auto border-t border-[rgba(31,42,31,0.08)] pt-4">
        ...
        {learnerNavItems.map((item) => {
          ...
          return (
            <li key={item.href}>
              <a
                href={item.href}
                className={`flex min-h-12 items-center justify-center rounded-2xl px-3 text-sm font-medium ${
                  active
